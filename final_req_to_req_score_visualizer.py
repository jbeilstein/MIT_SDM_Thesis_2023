...
def requirement_to_requirement_score_visualizer(requirement_file_name):

    original_requirements_dataframe = pd.read_csv(requirement_file_name,sep=',')
    ...
    axes.set_xticklabels(req_column_names[1:(len(req_column_names))])
    plot.xticks(rotation=90,fontsize=5)
    axes.yaxis.set_major_locator(ticker.FixedLocator(range(num_classes-1)))
    axes.set_yticklabels(req_column_names[1:(len(req_column_names))]) 
    ...
    ontology_dsm = plot.savefig('req_to_req_dsm_two_way_normalized.jpg', dpi=1000)
    print('Requirement to requirement visual comparisons completed')