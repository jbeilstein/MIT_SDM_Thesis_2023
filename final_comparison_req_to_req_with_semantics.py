...
#This module will semantically compare every requirement statement to each other.  It will use the 'all-MiniLM-L6-v2' language model to semantically compare
#the two entries using cosine similarity.  Every requirement statement will be compared to every requirement statement. The results of this process are 
#stored in 'final_output_req_to_req_pairing_comparison.csv'.  Note that this module is capable of performing token to token comparisons using spacy.
#This functionality has been disabled due to excessive high false positive rates, but the functionality remains.

def requirement_to_requirement_comparison_tool(requirement_file_name):
    ...
    #Cycle through every requirement
    for req_i, req_row in requirements_dataframe.iterrows():
        if (req_row['id_num'] != '') and (req_row['id_num'] != 'id_num') and (math.isnan(req_row['id_num']) != True):
            ...
            if (req_row['id_num'] >= 0) and (req_row['entry_type'] != 'block'):
                ...
                #Cycle through every other requirement, and do not compare to self.
                for req2_i, req2_row in requirements_dataframe.iterrows():
                    if (req2_row['id_num'] != '') and (req2_row['id_num'] != req_row['id_num']) and (math.isnan(req2_row['id_num']) != True) and (req2_row['entry_type'] != 'block'):                    
                        ...
                        if (req_row['id_num'] >= 0) and (req2_i > req_i):
                            ...
                            #     if (str(r_token) not in unwanted_str) and (str(r_token) not in common_articles) and (str(r_token) != '\n') and (r_token.dep_ != 'nummod') and (r_token.dep_ != 'punct') and (r_token.dep_ != 'appos') and (str(r_token) != '') and (str(r_token) != ' '):    
                            #         for r2_token in req2_doc:
                            #             if (str(r2_token) not in unwanted_str) and (str(r2_token) not in common_articles) and (str(r2_token) != '\n') and (r2_token.dep_ != 'nummod') and (r2_token.dep_ != 'punct') and (r2_token.dep_ != 'appos') and (str(r_token) != '') and (str(r_token) != ' '):
    ...
    req_pairing.to_csv('final_output_req_to_req_pairing_comparison.csv', index=False)
    ...
    print('Requirement to requirement semantic comparison complete')
